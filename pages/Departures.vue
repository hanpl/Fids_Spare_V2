        ...
        <table class="table table-hover" id="tblWorkOrderDone">
          <thead class="classtheadheader" style="background-color: #36c0c7; height: 6vh;">
            <tr>
              ...
              <th style="width: 22vw;"><span style="float: left; padding-left: 2.4vw;">DESTINATION</span></th>
...
interface Flight {
  ...
  counterStart: string; counterEnd: string; lineCode: string; flight: string; city: string
  gate: string; remark: string; checkInCounters: string; codeShare: string
}
...
const loadFlights = async () => {
  try {
    const data = await $fetch<Flight[]>(`${config.public.apiBase}/api/FidsDepartures`, {
...
.sizerow:nth-child(odd) { background-color: #283b92; }
.sizerow:nth-child(even) { background-color: #121441; }
.table td, .table th {
  padding: 0.25rem;
