...
const connectHub = async () => {
  const url = `${config.public.urlHub}`;
...
const useFlightSorting = async (top: number,  rollOn: number,  rollOff: number,  location: string,  numberM: number): Promise<FlightInfo[]> => 
{
  ...
  const { data, error } = await useFetch<FlightInfo[]>('/SortingDep', {
...
const loadconfig = async () => {
  try {
    const response = await fetch(`${config.public.apiBase}/Sorting`);
...
const intervalLoadShort = ref<ReturnType<typeof setInterval> | null>(null);
onMounted(async () => {
    await loadAll()
    startPolling()
    await loadCityMap();
